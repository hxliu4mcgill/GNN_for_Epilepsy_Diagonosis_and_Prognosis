...
# from model_multihop_Markov import *
from dataset import *
...
def step(model, criterion, dyn_v, dyn_a, sampling_endpoints, t, label, reg_lambda, clip_grad=0.0, device='cpu', optimizer=None, scheduler=None, con_criterion=None):
    ...
    if optimizer is not None:
        ...
        con_loss = con_criterion(latent, label.to(device))
        loss = criterion(logit, label.to(device)) + alpha * con_loss
...
def train(argv):
    ...
    for k in range(checkpoint['fold'], argv.k_fold):
        ...
        # 定义损失函数并应用类别权重
        criterion = nn.CrossEntropyLoss(weight=torch.Tensor([weight_negative, weight_positive]).to(device),
                                        label_smoothing=0.1)
        criterion2 = losses.ClipConstrastLoss()
