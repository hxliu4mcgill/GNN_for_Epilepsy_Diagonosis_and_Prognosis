...
def step(model, criterion, dyn_v, dyn_a, sampling_endpoints, t, label, reg_lambda, clip_grad=0.0, device='cpu', optimizer=None, scheduler=None, adaptation_steps=1):
    global nways, kshot
    ...
    if adaptation_steps > 0:
        with torch.backends.cudnn.flags(enabled=False):
            ...
            dyn_v, dyn_a, t, label = dyn_v.to(device), dyn_a.to(device), t.to(device), label.to(device)

            adaptation_indices = np.zeros(dyn_v.size(0), dtype=bool)
            adaptation_indices[np.arange(kshot * nways) * 2] = True
            evaluation_indices = torch.from_numpy(~adaptation_indices)
            adaptation_indices = torch.from_numpy(adaptation_indices)
            adaptation_v, adaptation_a, adaptation_labels,  = dyn_v[adaptation_indices], dyn_a[adaptation_indices], label[adaptation_indices]
            evaluation_v, evaluation_a, evaluation_labels,  = dyn_v[evaluation_indices], dyn_a[evaluation_indices], label[evaluation_indices]

            for step in range(adaptation_steps):
                logit, attention, latent, reg_ortho = model(adaptation_v, adaptation_a, t[:,adaptation_indices,...], sampling_endpoints)
                loss = criterion(logit, adaptation_labels)
                model.adapt(loss)
            # reg_ortho *= reg_lambda

            logit, attention, latent, reg_ortho = model(evaluation_v, evaluation_a, t[:,evaluation_indices,...], sampling_endpoints)
            evaluation_error = criterion(logit, evaluation_labels)
            return logit, evaluation_error, attention, latent, reg_ortho, evaluation_labels
    else:
        logit, attention, latent, reg_ortho = model(dyn_v.to(device), dyn_a.to(device), t.to(device),
                                                    sampling_endpoints)
...
def train(argv):
    ...
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=16, shuffle=False, num_workers=4, pin_memory=True)
    ...
    for k in range(checkpoint['fold'], argv.k_fold):
        ...
        meta_trainval = learn2learn.data.MetaDataset(dataset)

        transforms = [
            learn2learn.data.transforms.NWays(meta_trainval, nways),  # Samples N random classes per task (here, N = 5)
            learn2learn.data.transforms.KShots(meta_trainval, 2*kshot),
            # Samples K samples per class from the above N classes (here, K = 1)
            learn2learn.data.transforms.LoadData(meta_trainval),  # Loads a sample from the dataset
            # Randomly rotate sample over x degrees (only for vision tasks)
        ]
        taskset = learn2learn.data.TaskDataset(dataset=meta_trainval, task_transforms=transforms,
                                               num_tasks=num_tasks)  # Creates sets of tasks from the dataset
        X, y = taskset.sample()
        ...
        fast_lr = 0.5
        maml_model = learn2learn.algorithms.MAML(model, lr=fast_lr)
        optimizer = torch.optim.Adam(maml_model.parameters(), lr=argv.lr, weight_decay=1e-5)
        ...
        for epoch in range(checkpoint['epoch'], argv.num_epochs):
            ...
            for i, x in enumerate(tqdm(taskset, ncols=60, desc=f'k:{k} e:{epoch}')):
                # process input data
                timeseries, label = x
                ...
                logit, loss, attention, latent, reg_ortho, evaluation_labels = step(
                    model=learner,
                    criterion=criterion,
                    dyn_v=dyn_v,
                    dyn_a=dyn_a,
                ...
                logger.add(k=k, pred=pred.detach().cpu().numpy(), true=evaluation_labels.detach().cpu().numpy(),
                           prob=prob.detach().cpu().numpy())
                summary_writer.add_scalar('lr', scheduler.get_last_lr()[0], i + epoch * len(taskset))
            ...
            summary_writer.add_scalar('loss', loss_accumulate/len(taskset), epoch)
            summary_writer.add_scalar('reg_ortho', reg_ortho_accumulate/len(taskset), epoch)
            ...
            for i, x in enumerate(tqdm(dataloader, ncols=60, desc=f'k:{k} e:{epoch}')):
                with torch.no_grad():
                    # process input data
                    timeseries, label = x
                    dyn_a, sampling_points = util.bold.process_dynamic_fc(timeseries, argv.window_size,
                                                                          argv.window_stride)
                    sampling_endpoints = [p + argv.window_size for p in sampling_points]
