...
def train(argv):
    ...
    for k in range(checkpoint['fold'], argv.k_fold):
        ...
        optimizer = torch.optim.Adam(maml_model.parameters(), lr=argv.lr)
        criterion = torch.nn.CrossEntropyLoss()

        # define optimizer and learning rate scheduler
        # optimizer = torch.optim.Adam(model.parameters(), lr=argv.lr, weight_decay=1e-3)
        ...
        for epoch in range(checkpoint['epoch'], argv.num_epochs):
            ...
            for i, x in enumerate(tqdm(dataloader, ncols=60, desc=f'k:{k} e:{epoch}')):
                # split task
                outer_loss = []
                k_shots = StratifiedKFold(argv.k_shots, shuffle=True, random_state=0)
                k_shot_generator = k_shots.split(x['idx'], x['label'])
                origin_weight = model.state_dict().copy()
                for _, task in k_shot_generator:

                    samples_num = 0
                    # split train and test in each task
                    task_samples_idx = x['idx'][task]
                    tr_te_split = StratifiedKFold(2, shuffle=True, random_state=0)
                    tr_te_generator = tr_te_split.split(task_samples_idx, x['label'][task_samples_idx])
                    train_idx, test_idx = list(tr_te_generator)[0]
                    train_sub_idx, test_sub_idx = task_samples_idx[train_idx], task_samples_idx[test_idx]

                    # process input data
                    dyn_a, sampling_points = util.bold.process_dynamic_fc(x['timeseries'][train_sub_idx], argv.window_size, argv.window_stride, argv.dynamic_length)
                    ...
                    t = x['timeseries'][train_sub_idx].permute(1,0,2)
                    label = x['label'][train_sub_idx]
                    # meta-train stage

                    inner_logit, inner_loss, attention, latent, reg_ortho = step(
                        model=model,
                        criterion=criterion,
                        dyn_v=dyn_v,
                        dyn_a=dyn_a,
                    ...
                    fast_weights = collections.OrderedDict((name, param - argv.inner_lr * grads)
                                                           for ((name, param), grads) in zip(fast_weights.items(), grads))
                    model.load_state_dict(fast_weights,strict=False)
                    # meta-val stage
                    dyn_a, sampling_points = util.bold.process_dynamic_fc(x['timeseries'][test_sub_idx], argv.window_size, argv.window_stride,argv.dynamic_length)
                    sampling_endpoints = [p + argv.window_size for p in sampling_points]
                    if i == 0: dyn_v = repeat(torch.eye(dataset.num_nodes), 'n1 n2 -> b t n1 n2',t=len(sampling_points), b=argv.minibatch_size)
                    if len(dyn_a) < argv.minibatch_size: dyn_v = dyn_v[:len(dyn_a)]
                    t = x['timeseries'][test_sub_idx].permute(1, 0, 2)
                    label = x['label'][test_sub_idx]
                    ...
                    samples_num += len(test_sub_idx)
                if optimizer is not None:
                    model.train()
                    model.load_state_dict(origin_weight)
                    optimizer.zero_grad()
                    outer_loss = torch.stack(outer_loss).sum() / samples_num
                    outer_loss.backward()
                    optimizer.step()
                    if scheduler is not None:
                        scheduler.step()
                ...
                loss_accumulate += outer_loss.detach().cpu().numpy()
