...
class MarkovClustering(nn.Module):
    def __init__(self, r, threshold, sparse):
        super(MarkovClustering, self).__init__()
        self.r=r
        self.threshold=threshold
        self.sparse=sparse
    def forward(self, m_, m_g): # m_: (b t) n n    m_b: (b t) n n
        if not self.sparse:
            m_ = torch.bmm(m_, m_g) # regulization
        else:
            m_ = torch.sparse.mm(m_, m_g)
        m_ = torch.div(m_.pow(self.r), torch.sum(m_.pow(self.r), dim=1, keepdim=True) + 1e-12)# inflation
        m_ = torch.nn.functional.threshold(m_, self.threshold, 0.0) # prune
        return m_

class MultiHopGNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, hop_num, r, threshold, feat_drop, sparse=False):
        super(MultiHopGNN,self).__init__()
        self.hop_num = hop_num
        self.feat_drop = nn.Dropout(feat_drop)
        self.sparse = sparse
        self.MarkovClustering = MarkovClustering(r=r, threshold=threshold, sparse=self.sparse)
        self.mlp = nn.Sequential(nn.Linear(input_dim, hidden_dim), nn.BatchNorm1d(hidden_dim), nn.ReLU(),
    ...
    def forward(self, A, X, t, b, n, L1_norm=True):
        '''
        In- A:(b t) n c   X:(b t) n c
        Out- out:(b t n) c
        '''
        if not self.sparse:
            ...
            # for n in A:
            #     a = torch.diag(n)
            #     if torch.sum(a) != num_nodes:
            #         print("not 1")
            #         exit()
            d = torch.sum(A, dim=-1, keepdim=False)
            d_ = torch.diag_embed(torch.pow(d, -1.0))
            m_g = torch.bmm(A, d_)
            m_0 = m_g.clone()
            m_out = m_g.clone()
            m_k = self.MarkovClustering(m_0, m_g) # m_1
            h_1 = torch.bmm(m_k.permute(0, 2, 1), X)
            h_k = h_1.clone()# M^T * X  h_1
            m_out += m_k
            for _ in range(1, self.hop_num - 1):
                m_k = self.MarkovClustering(m_k, m_g)
                h_k = torch.bmm(m_k.permute(0, 2, 1), h_k)
                m_out += m_k
            m_k = self.MarkovClustering(m_k, m_g)
            h_k = torch.bmm(m_k.permute(0, 2, 1), h_k) + h_1 + X
            m_out += m_k
            if L1_norm:
                l1_norm_cols = torch.norm(h_k, p=1, dim=1, keepdim=True) + 1e-12
                feat = torch.div(h_k, l1_norm_cols)
            ...
            d = torch.sum(A, dim=-1, keepdim=False)
            d_val = torch.pow(d, -1.0).to_dense()
            d_idx = torch.as_tensor([list(range(d.shape[0])), list(range(d.shape[0]))]).to('cuda:0')
            d_ = torch.sparse.FloatTensor(d_idx, d_val)

            m_g = torch.sparse.mm(A, d_)
            m_0 = m_g.clone()
            m_out = m_g.clone()
            m_k = self.MarkovClustering(m_0, m_g)  # m_1
            h_1 = torch.bmm(m_k.permute(0, 2, 1), X)
...
class ModelSTAGIN(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes, num_heads, num_layers, sparsity, dropout=0.5, cls_token='sum', readout='sero', hop_num=4, garo_upscale=1.0):
        ...
        self.dropout = nn.Dropout(dropout)
        self.multi_hop_gnn = nn.ModuleList()

        self.threshold = 0.1

        for i in range(num_layers):
            self.multi_hop_gnn.append(MultiHopGNN(hidden_dim, hidden_dim, hop_num, r=2, threshold=0.01, feat_drop=0.2)) # input_dim, hidden_dim, hop_num, r, threshold, feat_drop
    ...
    def forward(self, v, a, t, sampling_endpoints):
        ...
        for layer, (M, R, T, L) in enumerate(zip(self.multi_hop_gnn, self.readout_modules, self.transformer_modules, self.linear_layers)):
            h_bridge = rearrange(h, '(b t n) c -> (b t) n c', t=num_timepoints, b=minibatch_size, n=num_nodes)

            h, edge_attn = M(a, h_bridge, t=num_timepoints, b=minibatch_size, n=num_nodes)
