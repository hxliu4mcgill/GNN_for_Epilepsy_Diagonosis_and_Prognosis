...
def train(argv):
    ...
    for k in range(checkpoint['fold'], argv.k_fold):
        ...
        # optimizer = torch.optim.SGD(model.parameters(), lr=argv.lr)
        # inner_optimizer = torch.optim.SGD(model.parameters(), lr=argv.inner_lr)
        optimizer = torch.optim.Adam(model.parameters(), lr=argv.lr, weight_decay=0.5*1e-3)
