...
def step(model, criterion, dyn_v, dyn_a, sampling_endpoints, t, label, reg_lambda, clip_grad=0.0, device='cpu', optimizer=None, scheduler=None, meta_train=True):
    ...
    if optimizer is not None:
        ...
        pretu_v, pretu_a = dyn_v_, dyn_a_
        ori_v, ori_a = dyn_v_.data, dyn_a_.data
        for i in range(iters):
            pretu_v.requires_grad = True
            pretu_a.requires_grad = True
            logit, attention, latent, reg_ortho = model(pretu_v, pretu_a, t_, sampling_endpoints)
            loss = criterion(logit, label_)
            model.zero_grad()
            loss.backward()
            adv_x = pretu_v + alpha * torch.sign(pretu_v.grad.data)
            adv_z = pretu_a + alpha * torch.sign(pretu_a.grad.data)
            eta_x = torch.clamp(adv_x - ori_v, min=-eps, max=eps)
            eta_z = torch.clamp(adv_z - ori_a, min=-eps, max=eps)
            pretu_v = torch.clamp(ori_v + eta_x, min=0, max=1).detach_()
            pretu_a = torch.clamp(ori_a + eta_z, min=-1, max=1).detach_()

        y, attention, latent, reg_ortho = model(pretu_v, pretu_a, t_, sampling_endpoints)
        yy, attention, latent, reg_ortho = model(dyn_v_, dyn_a_, t_, sampling_endpoints)
        loss = 0.5 * (criterion(yy, label_) + criterion(y, label_))
